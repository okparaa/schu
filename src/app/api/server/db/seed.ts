...
import { BulletinsSchema } from "../schemas/bulletins.schema";
...
export async function seed() {
  ...
  const assignmentsData: (typeof assignments.$inferInsert)[] = [];
  const resultsData: (typeof results.$inferInsert)[] = [];
  const attendanceData: (typeof attendance.$inferInsert)[] = [];
  const bulletinsData: (typeof bulletins.$inferInsert)[] = [];
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const user = generateMock(UsersSchema, {
      stringMap: {
        ...
        email: () =>
          faker.internet
            .email({ firstName: surname, lastName: lastname })
            .toLowerCase(),
        username: () =>
  ...
  console.log("seeding table: students/users");
  ...
  for (let i = 0; i < 30; i++) {
    ...
    student.classId = classData[i % classData.length].id as string;
    student.gradeId = gradesData[i % gradesData.length].id as string;
    usersData.push(usr);
  ...
  for (let i = 0; i < 50; i++) {
    ...
    lesson.classId = classData[i % classData.length].id as string;
  ...
  for (let i = 0; i < 14; i++) {
    const assignment = generateMock(AssignmentsSchema);
    assignment.startTime = dayjs().day(i).hour(3).toDate().toString();
    assignment.dueDate = dayjs()
      .day(i + 2)
      .hour(3)
      .toDate();
    assignment.lessonId = lessonsData[i % lessonsData.length].id as string;
    const [asign] = await db.insert(assignments).values(assignment).returning();
    assignmentsData.push(asign);
  }
  ...
  console.log("seeding table: bulletins");
  for (let i = 0; i < 40; i++) {
    const bulletin = generateMock(BulletinsSchema);
    bulletin.classId = classData[i % classData.length].id as string;
    bulletin.date = dayjs().day(i).toDate();
    const [bullet] = await db.insert(bulletins).values(bulletin).returning();
    bulletinsData.push(bullet);
  }
  console.log("seeding table: attendance");
  for (let i = 0; i < 30; i++) {
    const attend = generateMock(AttendanceSchema);
    attend.lessonId = lessonsData[i % lessonsData.length].id as string;
    attend.date = dayjs().day(i).toDate();
    attend.studentId = studentsData[i % studentsData.length].id as string;
    const [tend] = await db.insert(attendance).values(attend).returning();
    attendanceData.push(tend);
  }
  console.log("seeding table: results");
  for (let i = 0; i < 14; i++) {
    const result = generateMock(ResultsSchema, {
      stringMap: {
        score: () => faker.finance.amount({ min: 10, max: 100 }),
      },
    });
    result.assignmentId = assignmentsData[i].id as string;
    result.examId = examsData[i % examsData.length].id as string;
    result.studentId = studentsData[i].id as string;

    const [res] = await db.insert(results).values(result).returning();
